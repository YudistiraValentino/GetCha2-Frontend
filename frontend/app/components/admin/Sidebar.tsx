...
export default function AdminSidebar({ isOpen }: { isOpen: boolean }) {
  ...
  return (
    <aside 
      className={`bg-navy-900 text-white h-screen fixed left-0 top-0 flex flex-col shadow-2xl z-50 transition-all duration-300 ease-in-out ${
        isOpen ? "w-64" : "w-20"
      }`}
    >
      {/* 1. Logo Area */}
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Tooltip Keren saat Collapsed (Optional) */}
              {!isOpen && (
                  <div className="absolute left-16 bg-navy-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 border border-gray-700 shadow-xl">
                      {item.name}
                  </div>
              )}
            </Link>
