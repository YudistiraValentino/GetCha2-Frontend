...
export default function ProfilePage() {
  ...
  const handleLogout = async () => {
    ...
    localStorage.clear(); // Hapus semua data (token, user, cart)
    
    setTimeout(() => {
        window.location.href = '/login'; // Fresh reload ke login
    }, 1000);
  };
    ...
    <main className="min-h-screen bg-[#F8F9FA] pb-32">
      ...
      <div className="container mx-auto px-4 md:px-12 pt-32 max-w-lg">
        <div className="flex justify-between items-end mb-6">
            ...
            <button className="text-sm font-bold text-gold-600">Edit</button>
        </div>

        <div className="bg-navy-900 rounded-[2rem] p-6 text-white shadow-xl mb-8 relative overflow-hidden group transition-all hover:scale-[1.02] duration-500">
            <div className="absolute top-0 right-0 w-48 h-48 bg-gold-500 rounded-full blur-[80px] opacity-20"></div>
            <div className="flex items-center gap-5 mb-8 relative z-10">
                <div className="w-20 h-20 bg-white/10 rounded-full flex items-center justify-center border-2 border-white/20 backdrop-blur-md"><User size={36} className="text-gold-400" /></div>
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">{user?.name || "Loading..."}</h2>
                    <p className="text-white/60 text-sm">{user?.email}</p>
                    <div className="mt-2"><span className="bg-gold-500/20 text-gold-400 px-2 py-0.5 rounded text-[10px] font-bold border border-gold-500/20 uppercase tracking-wide">@{user?.username || 'Member'}</span></div>
                </div>
            </div>
            ...
            <div className="bg-white/5 rounded-2xl p-5 backdrop-blur-md border border-white/10 relative z-10">
                <div className="flex justify-between items-end mb-2">
                    <div>
                        <p className="text-[10px] text-gold-400 uppercase font-bold tracking-widest mb-1 flex items-center gap-1"><Star size={10} fill="currentColor"/> Loyalty Points</p>
                        <p className="text-3xl font-black text-white">{user?.points ? user.points.toLocaleString() : 0}</p>
                    </div>
                </div>
                <div className="w-full bg-white/10 h-1.5 rounded-full mt-3 overflow-hidden"><div className="bg-gold-500 h-full w-[45%] rounded-full shadow-[0_0_10px_rgba(234,179,8,0.5)]"></div></div>
            </div>
        </div>

        <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center">
                 <div className="w-10 h-10 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mb-2"><ShoppingBag size={20} /></div>
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center">
                 <div className="w-10 h-10 rounded-full bg-green-50 text-green-600 flex items-center justify-center mb-2"><ReceiptText size={20} /></div>
                 <p className="text-lg font-black text-navy-900 line-clamp-1">{totalSpent > 0 ? `Rp${totalSpent.toLocaleString('id-ID')}` : 'Rp0'}</p>
                ...
                <div key={idx} onClick={() => handleNavigate(item.link)} className="cursor-pointer block hover:bg-gray-50 transition-colors group">
                    <div className="flex items-center justify-between p-5 border-b border-gray-50 last:border-0">
                        <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${item.bg} ${item.color} group-hover:scale-110 transition-transform`}><item.icon size={22} /></div>
                            <span className="font-bold text-navy-900 text-sm">{item.label}</span>
                        </div>
                        <div className="flex items-center gap-3">
                            {item.badge && <span className="bg-red-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-md">{item.badge}</span>}
        ...
        <button onClick={() => setShowLogoutModal(true)} className="w-full py-4 rounded-2xl border-2 border-red-50 text-red-500 font-bold flex items-center justify-center gap-2 hover:bg-red-50 transition-all active:scale-[0.98]"><LogOut size={20} /> Log Out</button>
      </div>

      {showLogoutModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-navy-900/40 backdrop-blur-sm" onClick={() => setShowLogoutModal(false)}></div>
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 relative z-10 animate-in zoom-in-95 border border-gray-100">
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <button onClick={handleLogout} className="py-3 bg-red-500 text-white rounded-xl font-bold">Logout</button>
            </div>
          </div>
        </div>
      )}
