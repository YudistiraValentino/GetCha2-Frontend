        ...
        try {
            ...
            if (jsonOrder.success) {
                ...
                const spent = jsonOrder.data
                    ...
                    .reduce((acc: number, curr: any) => acc + parseInt(curr.total_price || 0), 0);
                setTotalSpent(spent);
            }
    ...
    <main className="min-h-screen bg-[#F8F9FA] pb-32">
      ...
      <div className="container mx-auto px-4 md:px-12 pt-32 max-w-lg">
        ...
        <div className="bg-navy-900 rounded-[2rem] p-6 text-white shadow-xl shadow-navy-900/20 mb-8 relative overflow-hidden group transition-all hover:scale-[1.02] duration-500">
            <div className="absolute top-0 right-0 w-48 h-48 bg-gold-500 rounded-full blur-[80px] opacity-20 translate-x-10 -translate-y-10"></div>
            ...
            <div className="flex items-center gap-5 mb-8 relative z-10">
                ...
                <div>
                    {/* Menggunakan Optional Chaining agar tidak error saat user null */}
                    ...
                    <div className="flex gap-2 mt-2">
                         <span className="bg-gold-500/20 text-gold-400 px-2 py-0.5 rounded text-[10px] font-bold border border-gold-500/20 uppercase">
            ...
            <div className="bg-white/5 rounded-2xl p-5 backdrop-blur-md border border-white/10 relative z-10">
                ...
                <div className="w-full bg-white/10 h-1.5 rounded-full mt-3 overflow-hidden">
                    <div className="bg-gold-500 h-full w-[45%] rounded-full"></div>
                </div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center">
                 ...
                 <p className="text-lg font-black text-navy-900">
                     {totalSpent > 0 ? `Rp${(totalSpent/1000).toFixed(0)}k` : 'Rp0'}
                 </p>
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden mb-8">
            {menuItems.map((item, idx) => (
                <div key={idx} onClick={() => handleNavigate(item.link)} className="cursor-pointer hover:bg-gray-50 transition-colors">
        ...
        <button onClick={() => setShowLogoutModal(true)} className="w-full py-4 rounded-2xl border-2 border-red-50 text-red-500 font-bold flex items-center justify-center gap-2 hover:bg-red-50">
            <LogOut size={20} /> Log Out
        </button>
      </div>

      {/* LOGOUT MODAL (Sama seperti sebelumnya) */}
      {showLogoutModal && (
