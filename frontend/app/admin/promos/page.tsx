...
export default function AdminPromosList() {
  ...
  return (
    <div>
        {/* HEADER SECTION (Menyesuaikan style Admin Panel) */}
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            ...
            <div className="flex gap-3">
                <button onClick={fetchPromos} className="bg-white text-navy-900 px-4 py-2 rounded-xl shadow-sm border border-gray-200 flex items-center gap-2 hover:bg-gray-50 transition text-sm font-bold">
                    <RefreshCw size={16} className={loading ? "animate-spin" : ""} /> Refresh
                </button>
                
                <Link href="/admin/promos/create" className="bg-navy-900 text-white px-5 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-gold-500 hover:text-navy-900 transition shadow-lg text-sm">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-8">
            <div className="relative max-w-md">
                ...
                <input 
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:border-navy-900 transition focus:bg-white text-sm"
                    ...
                    <div key={promo.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col group hover:shadow-md transition">
                        ...
                        <div className="h-40 bg-gray-100 relative">
                            ...
                            <div className="absolute top-3 right-3">
                                {promo.is_active === 1 ? (
                                    <span className="bg-green-500 text-white text-[10px] font-bold px-2 py-1 rounded-full flex gap-1 items-center shadow-sm"><CheckCircle size={10}/> ACTIVE</span>
                                ) : (
                                    <span className="bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded-full flex gap-1 items-center shadow-sm"><XCircle size={10}/> INACTIVE</span>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="p-5 flex-1 flex flex-col">
                            ...
                            <div className="flex items-center gap-2 text-xs text-gray-400 mb-4 bg-gray-50 p-2 rounded w-fit">
                                <Calendar size={14}/><span>{promo.start_date}</span> - <span>{promo.end_date}</span>
                            </div>

                            <div className="pt-4 border-t border-gray-100 flex justify-between items-center mt-auto">
                                <div className="text-sm font-bold text-blue-600">
                                    {promo.type === 'fixed' ? `Rp ${parseInt(promo.discount_amount).toLocaleString()}` : `${promo.discount_amount}% OFF`}
                                </div>
